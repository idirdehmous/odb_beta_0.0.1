...
PUBLIC int pyodb(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc, argv, FLAGS)) != -1) {
    ...
    case 'o':
      ...
      printf("Data to be written in outfile %s",   outfile ) ; 
      break;
    case 'p':
	...
	for (i=0; i<nd; i++) {
	  ...
	  if (print_mdi && pci->dtnum != DATATYPE_STRING && ABS(d[i]) == mdi) {
		...
		*scc = '\0';
	    ...
	    case DATATYPE_INT4:
	      //printf("%d,",  (int) d[i]) ;
	      //enough=(int)((ceil(log10( (int)d[i]))+1)*sizeof(char)) ;
